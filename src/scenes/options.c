...
static const char *options_scene_bitmap_get_value(s32 entry) {
    switch (entry) {
        ...
        case OPTIONS_BITMAP_SKIP_DISCLAIMER:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_SKIP_DISCLAIMER) ? "ÇrÇÅÇåÇîÇÅÇí" : "ÇlÇèÇìÇîÇíÇÅÇí";

        case OPTIONS_BITMAP_ALT_GAME_SELECT_MUSIC:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_USE_ALT_GAME_SELECT_MUSIC) ? "ÇrÇâ" : "ÇmÇè";
    }

    return "ÇrÇgÇ`ÇeÇeÇxÅ@ÇkÇ`Å@ÇbÇ`ÇfÇn";
}
