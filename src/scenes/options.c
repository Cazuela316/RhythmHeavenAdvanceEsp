...
static const char *options_scene_bitmap_get_value(s32 entry) {
    switch (entry) {
        case OPTIONS_BITMAP_NON_JP_SFX:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_NON_JP_SFX) ? "‚h‚Ž‚‡‚Œ‚…‚“" : "‚i‚‚‚‚Ž‚…‚“";

        case OPTIONS_BITMAP_NON_JP_MUSIC:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_NON_JP_MUSIC) ? "‚h‚Ž‚‡‚Œ‚…‚“" : "‚i‚‚‚‚Ž‚…‚“";
#ifdef RUMBLE
        case OPTIONS_BITMAP_RUMBLE:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_DISABLE_RUMBLE) ? "‚m‚" : "‚r‚‰";
#endif
        case OPTIONS_BITMAP_SKIP_DISCLAIMER:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_SKIP_DISCLAIMER) ? "‚r‚‚Œ‚”‚‚’" : "‚l‚‚“‚”‚’‚‚’";

        case OPTIONS_BITMAP_ALT_GAME_SELECT_MUSIC:
            return CHECK_ADVANCE_FLAG(D_030046a8->data.advanceFlags, ADVANCE_FLAG_USE_ALT_GAME_SELECT_MUSIC) ? "‚r‚‰" : "‚m‚";
    }

    return "‚r‚g‚`‚e‚e‚x@‚k‚`@‚b‚`‚f‚n";
}
...
static void options_scene_bitmap_build_line_text(s32 line, u32 lineBuffer) {
    ...
    snprintf(gOptionsMenu->bitmapLineBuffer[lineBuffer], OPTIONS_BITMAP_LINE_BUFFER_SIZE, "%sF@%s",
